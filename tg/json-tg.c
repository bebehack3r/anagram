...
void json_pack_user (json_t *res, tgl_peer_t *P) {
  ...
  assert (json_object_set (res, "event", json_string ("resolve")) >= 0);
}
...
void json_pack_channel (json_t *res, tgl_peer_t *P) {
  ...
  if (P->channel.username) {
    printf ("%s", P->channel.username);
    assert (json_object_set (res, "channel_name", json_string (P->channel.username)) >= 0);
  }
...
json_t *json_pack_media (struct tgl_message_media *M) {
  ...
  case tgl_message_media_photo:
    ...
    //if (M) {
    //  printf("%u",M->type);
    //  assert (json_object_set (res, "others", json_string (M->webpage->url)) >= 0);
    //}
    break;
